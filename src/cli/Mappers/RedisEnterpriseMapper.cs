...
public class RedisEnterpriseMapper : IResourceCostMapper
{
    public string ResourceType => "Microsoft.Cache/redisEnterprise";
    ...
    public MonthlyCost CalculateCost(ResourceDescriptor resource, List<PriceRecord> prices)
    {
        ...
        var price = prices
            .Where(p => p.MeterName != null && p.MeterName.Contains("Enterprise") &&
                p.UnitOfMeasure == "1 Hour" && p.UnitPrice > 0)
    ...
    private static string GetSkuName(ResourceDescriptor resource)
    {
        if (resource.Sku.TryGetValue("name", out var name) && name.ValueKind == JsonValueKind.String)
            return name.GetString() ?? "Enterprise_E10";
        return "Enterprise_E10";
    }
